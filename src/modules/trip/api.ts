...
export async function getTrips(filters?: {
  ...
  let query = supabase.from('trips_with_participants').select('*').order('created_at', { ascending: false });
...
export async function getTripById(tripId: string): Promise<ApiResponse<Trip>> {
  const { data, error } = await supabase.from('trips_with_participants').select('*').eq('id', tripId).single();
  return { data, error };
}

export async function getTripsByOwner(ownerId: string): Promise<ApiResponse<Trip[]>> {
  const { data, error } = await supabase.from('trips').select('*').eq('owner_id', ownerId).order('created_at', { ascending: false });
  return { data, error };
}
...
export async function leaveTrip(tripId: string): Promise<ApiResponse<any>> {
  ...
  const { error } = await supabase.from('trip_participants').delete().eq('trip_id', tripId).eq('user_id', user.id);
  return { data: null, error };
}